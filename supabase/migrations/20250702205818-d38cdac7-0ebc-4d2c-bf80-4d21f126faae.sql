-- Final Migration Phase: Bookings with Corrected Status Mapping
...
SELECT 
    ...
    CASE 
        ...
        WHEN b.booking_status = 'submitted' THEN 'pending'
        WHEN b.booking_status = 'pending' THEN 'pending'
        WHEN b.booking_status = 'cancelled' THEN 'cancelled'
        WHEN b.booking_status = 'missed_booking' THEN 'cancelled'
        ELSE 'open'
    END as status,
    CASE 
    ...
    SUBSTRING(COALESCE(b.note, ''), 1, 500) as notes, -- Truncate long notes
...
VALUES ('final_migration_report', 'completed', 
        ...
        'Complete legacy data migration finished successfully');
...
SELECT 
    ...
    (SELECT COUNT(*) FROM legacy_id_mappings) as id_mappings,
    now() as completed_at;